...
export function applyOverride(object: ComfyApi) {
                    ...
                    if (message && message.internal) {
                        ...
                        if (
                            ...
                            if (internalMessage.type === "upload_done") {
                                resolve(
                                    createResponse({ name: file.name, subfolder: subdir }),
                                );
    ...
    object.fetchApi = async (path: string, options?: any): Promise<Response> => {
        ...
        if (
            (path === "/upload/image" || path === "/upload/mask")
        ) {
    ...
    async function updateNodeDefinitions() {
        ...
        for (let nodeId in app.graph._nodes) {
        ...
        for (const nodeType in definitions) {
            ...
            if (Array.isArray(definition) && Array.isArray(definition[0])) {
                if (nodeType == "video" || nodeType == "default_video") {
                    ...
                    const { publicContents, privateContents } = getFromFolderFiles("inpaint");
                    definitions[nodeType] = [
                        [
                            ...privateContents,
                            ...publicContents,
                ...
                for (let i = 0; i < definition[0].length; i++) {
                    ...
                    // --------------- //
                        ...
                        const { publicContents, privateContents } = getFromFolderFiles("unet");
                        const { publicContents: diffusion_models_publicContents, privateContents: diffusion_models_privateContents } = getFromFolderFiles("diffusion_models");
                        definitions[nodeType] = [
                            [
                                ...definition[0].slice(0, i),
                                ...privateContents,
                        ...
                        const { publicContents: llm_gguf_publicContents, privateContents: llm_gguf_privateContents } =
                        ...
                        definitions[nodeType] = [
                            [
                                ...definition[0].slice(0, i),
                                ...privateContents.map((item) => `bbox/${item.replace('ultralytics/', '')}`),
                                ...publicContents.map((item) => `bbox/${item.replace('ultralytics/', '')}`),
                            ],
                            ...definition.slice(1),
                        ];
                    } else if (definition[0][i] === "bbox/__ultralytics_bbox__") {
