...
// const { ComfyWidgets, LGraphNode } = window.app.widgets;
// const { ComfyDialog, $el } = window.app.ui;
import { app } from '../app'
import { api } from '../api'
import { LGraphNode, LiteGraph } from '@comfyorg/litegraph'
import { ComfyDialog, $el } from '../ui'
...
function dispatchAPIEventData(data) {
  ...
  switch (msg.event) {
    ...
    case 'progress':
      ...
      break
    case 'executing':
      api.dispatchEvent(new CustomEvent('executing', { detail: msg.data.node }))
      break
    case 'executed':
...
function convertToInput(node, widget, config) {
  ...
  if (type === 'STRING') {
    const inputNode = LiteGraph.createNode('ComfyUIDeployExternalText')
    console.log(widget)
    const index = node.inputs.findIndex((x) => x.name === widget.name)
    console.log(node.widgets_values, index)
    inputNode.configure({
    ...
    inputNode.pos = [...node.pos]
    inputNode.pos[0] -= inputNode.size[0] + 40

    inputNode.connect(0, node, index)
    // inputNode.setDirtyCanvas(true, true);
  } else if (type === 'FLOAT') {
    const inputNode = LiteGraph.createNode('ComfyUIDeployExternalNumber')
    const index = node.inputs.findIndex((x) => x.name === widget.name)

    inputNode.configure({
      widgets_values: ['input_number', widget.value]
    })
    ...
    const inputNode = LiteGraph.createNode('ComfyUIDeployExternalNumberInt')
    const index = node.inputs.findIndex((x) => x.name === widget.name)
